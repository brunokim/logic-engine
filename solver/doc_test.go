...
func Example() {
	...
	s, _ := solver.NewSolver(clauses(
		...
		// add(s(A), B, s(Sum)) :-
		//   add(A, B, Sum).
		clause(comp("add", int_(0), var_("Sum"), var_("Sum"))),
		...
		clause(comp("mul", int_(0), var_("_"), int_(0))),
	...
	solutions, _ := s.Query(
		comp("mul", succ(succ(int_(0))), succ(succ(succ(int_(0)))), var_("Y")),
		comp("add", succ(int_(0)), var_("X"), var_("Y")))
